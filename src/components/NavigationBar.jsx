...
export function NavigationBar({
  ...
  onCloseGame, // Close the run and return to the games list
  unexploredCount, // reachable exits with no pairing yet
  onShowExits
}) {
  const [currentMapStats, setCurrentMapStats] = useState({
    ...
    battlegrounds: { cleared: 0, total: 0 }
  });
  ...
  useEffect(() => {
    ...
    const doors = locations.filter(loc => loc.type === 'door');
    ...
    const clearedBattlegrounds = battlegrounds.filter(bg =>
      locationTrackerService.getLocationState(gameId, bg.id)?.isOpened).length;
    ...
    setCurrentMapStats({
      ...
      battlegrounds: { cleared: clearedBattlegrounds, total: battlegrounds.length }
    });
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center">
        ...
        <div className="flex flex-col gap-2 items-center justify-end min-w-[280px] ml-8">
          ...
          <div className="flex gap-2">
            ...
            {onShowExits && (
              <button
                className="btn-nav px-3 py-2 text-sm bg-emerald-700 hover:bg-emerald-600 text-white"
                onClick={onShowExits}
                title="Exits you can reach but have not followed yet"
              >
                <span className="text-lg">🧭 {unexploredCount} left</span>
              </button>
            )}
        ...
        {gameId && currentFloorId && (
          <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-4 gap-2 text-xs text-center">
              ...
              <div className="bg-slate-700 rounded py-2 px-2">
                <div className="text-purple-400 font-bold text-lg">{currentMapStats.battlegrounds.cleared}/{currentMapStats.battlegrounds.total}</div>
                <div className="text-slate-400">Battlegrounds</div>
              </div>
            </div>
          </div>
        )}
        ...
        {gameId && (
          <div className="flex flex-col gap-2 items-center justify-end">
            ...
            {onCloseGame && (
              <button
                onClick={onCloseGame}
                className="px-3 py-2 text-sm font-medium rounded transition-colors bg-slate-700 hover:bg-slate-600 text-slate-300 mr-2"
                title="Close this run and go back to the games list"
              >
                <span className="text-lg">Close Run</span>
              </button>
            )}
            <button
